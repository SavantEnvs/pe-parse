...
: "${SRC:=/mayhem}"
# The library is a separate CMake build, so it needs the coverage half of the fuzzing
# engine explicitly: without it libFuzzer only sees the harness translation unit's own
# edges and fuzzes pe-parse blind (cov: 3, never reaches the resource parser).
: "${FUZZ_COV_FLAGS:=-fsanitize=fuzzer-no-link}"
export SANITIZER_FLAGS DEBUG_FLAGS CC CXX LIB_FUZZING_ENGINE MAYHEM_JOBS COVERAGE_FLAGS FUZZ_COV_FLAGS
...
cmake -S . -B build-fuzz \
  ...
  -DCMAKE_C_FLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS $FUZZ_COV_FLAGS" \
  -DCMAKE_CXX_FLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS $FUZZ_COV_FLAGS"
